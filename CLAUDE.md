...
- 사이트 글의 사진은 post.md 의 `![캡션](images/…)` 줄에서 온다 — **첫 사진이 썸네일**이 되고 본문에서는 빠진다(`prpub/site.py`). `::수치 …::` 요약 박스·URL 문단·`──` 구분선도 site.py 가 HTML 로 변환한다 (2026-09-04 mock 실측으로 보강). 목록 카드 요약(`summary`)은 meta.json 에 없으면 site.py 가 **본문 첫 문단**으로 채운다. 카드·목록 날짜(`conducted_at`)는 **게시일**이다 — 교육 일자는 본문에만 적는다
- 본문 끝 `함께 보면 좋은 글` + URL 블록은 네이버에선 에디터가 썸네일 카드로 바꾸지만 사이트는 글자 링크라, `frontend/src/app/(site)/cases/[slug]/page.tsx` 의 `splitRelatedBlock()` 이 그 블록을 본문에서 떼어 `/cases/<slug>` 카드(썸네일·제목·요약)로 다시 그린다. 블록이 없는 옛 수동 글에는 아무것도 붙지 않는다
