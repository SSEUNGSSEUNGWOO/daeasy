...
- 구독 수집은 세 경로(푸터 폼 / 가입 시 마케팅 동의 / 마이페이지 토글)로 이미 `newsletter_subscribers` 에 쌓이고 있었으나 **보내는 코드가 없었다**. `newsletter_issues` 는 스키마만 존재.
...
| 발신자 | `NEWSLETTER_FROM` (기본 `데이지 인사이트 <no-reply@daeasy.co.kr>`) | 문의 알림과 같은 Resend 인증 도메인 |
