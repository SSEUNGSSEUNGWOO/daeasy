...
| 발신자 | `NEWSLETTER_FROM` (기본 `DAEASY(데이지) <newsletter@daeasy.co.kr>`), 답장 주소 없음(도메인이 다르면 네이버·다음이 경고를 붙여 뺌 — newsletter@ 포워딩으로 대체 예정), 제목 `[DAEASY 뉴스레터] …` | 문의 알림과 같은 Resend 인증 도메인. no-reply 는 독자 답장(리드)이 반송돼 쓰지 않는다 |
