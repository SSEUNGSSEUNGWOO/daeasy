...
function Row({ row }: { row: ContactInquiryRow }) {
  ...
  async function handleStatusChange(next: InquiryStatus) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/inquiries/contact/${row.id}`, {
        method: "PATCH",
  ...
  return (
    <>
      <tr className={pending ? "opacity-60" : undefined}>
        ...
        <td className="px-4 py-3 text-zinc-600">{row.course?.title ?? "-"}</td>
        ...
        <tr className="bg-zinc-50/60">
          <td colSpan={6} className="px-4 py-4">
            <dl className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
              ...
              <Field label="소속" value={row.company || "-"} />
              <Field label="관심 과정" value={row.course?.title || "-"} />
