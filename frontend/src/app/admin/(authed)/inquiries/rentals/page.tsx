...
import { RentalInquiriesTable, type RentalInquiryRow } from "./inquiries-table";
...
async function fetchInquiries(status: InquiryStatus | "all"): Promise<RentalInquiryRow[]> {
  const sb = getSupabaseAdmin();
  let q = sb
    .from("rental_inquiries")
    .select("id, name, phone, usage_date, time_slot, message, status, created_at")
  ...
  return (data ?? []) as unknown as RentalInquiryRow[];
}
...
export default async function RentalInquiriesPage({
  ...
  return (
    <section>
      ...
      <div className="mt-6">
        <StatusFilter
          basePath="/admin/inquiries/rentals"
          current={status}
      ...
      <div className="mt-6">
        <RentalInquiriesTable inquiries={inquiries} />
      </div>
    </section>
  );
}
